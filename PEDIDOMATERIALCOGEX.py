...
st.markdown("**Sistema Web - Controle Matemático e Visual de Estoque - Método JIT e Kanban**")
...
def gerar_pedido(data_proximo_pedido, intervalo_novo_pedido):
    ...
    pedido_df['Status'] = pedido_df.apply(lambda row: '🔴 Crítico' if row['Estoque Atual'] < row['Estoque Necessário até Pedido'] else ('🟡 Alerta' if row['Estoque Atual'] < row['Estoque Necessário até Pedido'] + row['Consumo Médio Diário'] * intervalo_novo_pedido else '🟢 Ok'), axis=1)
...
if menu == "Pedido Automático de Material":
    st.header("📄 Pedido Automático de Material - Método JIT e Kanban")
...
# -------------------- RODAPÉ --------------------
st.markdown("---")
st.markdown("**COGEX ALMOXARIFADO - Gestão Matemática Just-In-Time & Kanban | Powered by Streamlit**")
